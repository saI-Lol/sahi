...
class SegmentAnythingModel(DetectionModel):
    ...
    @staticmethod
    def sam_to_yolo_like(sam_results):
        boxes = []
        masks = []
        ...
        for obj in sam_results:
            ...
            # Confidence (use predicted_iou or stability_score)
            conf = obj.get("predicted_iou", obj.get("stability_score", 1.0))

            # Class ID (SAM has no classes)
            cls = 0
    ...
    def perform_inference(self, image: np.ndarray):
        ...
        if self.has_mask:
            ...
            prediction_result = [
                self.sam_to_yolo_like(prediction_result)
            ]
        else:  # If model doesn't do segmentation or OBB then no need to check masks
            # We do not filter results again as confidence threshold is already applied above
            prediction_result = [self.sam_to_yolo_like(prediction_result)[0]]
    ...
    def category_names(self):
        if self.category_mapping:
            ...
            raise ValueError("Category names not available. Please provide category_mapping for SAM models.")
    ...
    def num_categories(self):
        ...
        if self.category_mapping:
            ...
            raise ValueError("Cannot determine number of categories. Please provide category_mapping for SAM models.")
